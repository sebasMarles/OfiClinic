...
export function keyFor(model: string): keyof typeof prisma {
  ...
  if (m === "clinicalhistory" || m === "clinicalhistories")
...
function repoOf(model: ModelName | string) {
  ...
  if (!repo) {
    console.error(
      `[DAL] delegate no encontrado para modelo "${model}" (key "${String(
        key
      )}")`
    );
  }
...
export async function findUnique(model: ModelName | string, id?: string) {
  ...
  if (!repo)
    throw new Error(
      `[DAL] findUnique(): delegate no encontrado para "${model}"`
    );
...
export async function create(model: ModelName | string, data: any) {
  const repo = repoOf(model);
  if (!repo)
    throw new Error(`[DAL] create(): delegate no encontrado para "${model}"`);
...
export async function update(model: ModelName | string, id: string, data: any) {
  if (!id)
    throw new Error(`[DAL] update() llamado sin id para modelo "${model}"`);
  const repo = repoOf(model);
  if (!repo)
    throw new Error(`[DAL] update(): delegate no encontrado para "${model}"`);
...
export async function remove(model: ModelName | string, id: string) {
  if (!id)
    throw new Error(`[DAL] remove() llamado sin id para modelo "${model}"`);
  const repo = repoOf(model);
  if (!repo)
    throw new Error(`[DAL] remove(): delegate no encontrado para "${model}"`);
...
export async function bulkDelete(model: ModelName | string, ids: string[]) {
  ...
  if (!repo)
    throw new Error(
      `[DAL] bulkDelete(): delegate no encontrado para "${model}"`
    );
...
export async function existsByField(
  ...
  if (!repo)
    throw new Error(
      `[DAL] existsByField(): delegate no encontrado para "${model}"`
    );

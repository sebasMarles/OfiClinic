...
export function GenericTable({
  ...
  // Exportar CSV
  const handleExport = () => {
    const selectedData = selectedRows.size > 0 ? data.filter(row => selectedRows.has(row.id)) : sortedData
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {config.rowActions && config.rowActions.length > 0 && <TableHead className='w-12' />}
              </TableRow>
            </TableHeader>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell
                    colSpan={visibleColumnConfigs.length + (config.enableSelection ? 1 : 0) + (config.rowActions?.length ? 1 : 0)}
                          ...
                          <DropdownMenuContent align='end'>
                            {config.rowActions.map(action => (
                              <DropdownMenuItem
                                ...
                                {action.icon === 'pencil' && <Pencil className='mr-2 h-4 w-4' />}
                                {action.icon === 'trash' && <Trash2 className='mr-2 h-4 w-4' />}
                                {action.label}
                              </DropdownMenuItem>
                            ))}
                          </DropdownMenuContent>

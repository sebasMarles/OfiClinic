...
import { schemaFromConfig } from "@/domain/validation/schema-from-config"; // <- ruta corregida
...
export function DynamicForm({
  ...
  const editableColumns = columns
    ...
    .filter((col) => !col.hidden && col.render !== "grid");
  ...
  const checkUnique = async (key: string, value: any) => {
    ...
    try {
      const url = `/api/crud/${(
        inferredSlug || ""
      ).toLowerCase()}/check-unique?field=${encodeURIComponent(
        key
      ...
      if (!res.ok) {
        setUnique(key, {
          ...
          conflict: false,
          message:
            submittedOnce || touched[key]
              ? data?.error || "No se pudo verificar"
              : undefined,
        });
      } else {
        setUnique(key, {
      ...
      setUnique(key, {
        ...
        conflict: false,
        message:
          submittedOnce || touched[key] ? "No se pudo verificar" : undefined,
      });
    }
  };
  ...
  const renderField = (column: ColumnConfig) => {
    ...
    const showHelp =
      (submittedOnce || touched[column.key]) && (ustate?.message || ferr);
        ...
        return (
          <>
            ...
            >
              ...
              >
                <SelectValue
                  placeholder={`Selecciona ${column.title.toLowerCase()}`}
                />
              </SelectTrigger>
              <SelectContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {editableColumns.map((column) => (
              <div
                key={column.key}
                className={column.type === "boolean" ? "col-span-full" : ""}
              >
                {column.type !== "boolean" && (
